...
const recipeCreateSchema = z.object({
  ...
  heroImageUrl: z.union([z.string().url(), z.string().min(1)]).optional(),
...
type RecipeCreatePayload = z.infer<typeof recipeCreateSchema>;
type RecipeUpdatePayload = z.infer<typeof recipeUpdateSchema>;
...
recipesRouter.post(
  ...
  async (req, res) => {
    const payload = req.body as RecipeCreatePayload;
    ...
    const recipe = await prisma.recipe.create({
      data: {
        ...
        publishedAt: null,
        ingredients: {
          create: payload.ingredients.map((ingredient: RecipeCreatePayload["ingredients"][number], index: number) => ({
            ...ingredient,
            position: index,
          })),
        },
        steps: {
          create: payload.steps.map((step: RecipeCreatePayload["steps"][number], index: number) => ({
            instruction: step.instruction,
            timerSeconds: step.timerSeconds,
            outcomeHint: step.outcomeHint,
            mediaAssetId: step.mediaAssetId,
            position: index,
          })),
        },
        tags: {
          create: payload.tags.map((tag: string) => ({ tag })),
        },
      },
      include: {
        ingredients: true,
...
recipesRouter.patch("/:recipeId", requireAuth, validateBody(recipeUpdateSchema), async (req, res) => {
  ...
  const payload = req.body as RecipeUpdatePayload;
