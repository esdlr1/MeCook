...
mediaRouter.post("/videos/sessions", requireAuth, requireRole([UserRole.VERIFIED_CREATOR, UserRole.ADMIN]), async (req, res) => {
    const { fileName, mimeType, totalBytes } = req.body;
...
mediaRouter.put("/videos/sessions/:sessionId/chunks/:chunkIndex", requireAuth, requireRole([UserRole.VERIFIED_CREATOR, UserRole.ADMIN]), rawChunkParser, async (req, res) => {
...
mediaRouter.post("/videos/sessions/:sessionId/complete", requireAuth, requireRole([UserRole.VERIFIED_CREATOR, UserRole.ADMIN]), async (req, res) => {
    const sessionId = String(req.params.sessionId);
    const session = await prisma.videoUploadSession.findUnique({ where: { id: sessionId } });
    if (!session || session.uploaderId !== req.authUser.id) {
        return res.status(404).json({ message: "Upload session not found" });
...
mediaRouter.get("/videos/sessions/:sessionId", requireAuth, requireRole([UserRole.VERIFIED_CREATOR, UserRole.ADMIN]), async (req, res) => {
    const sessionId = String(req.params.sessionId);
...
mediaRouter.post("/videos", requireAuth, requireRole([UserRole.VERIFIED_CREATOR, UserRole.ADMIN]), upload.single("video"), async (req, res) => {
    if (!req.file) {
